...
export default async function handler(req, res) {
  ...
  if (req.query.count !== '0') {
    ...
    // Visitas de hoje — guardadas por dia, mantidas 90 dias (para o gráfico)
    const dia = await redis('INCR', `portfolio:dia:${hoje}`);
    if (dia !== null) {
      await redis('EXPIRE', `portfolio:dia:${hoje}`, 7776000); // 90 dias
      resposta.visitasHoje = Number(dia);
    }
  } else {
    const total = await redis('GET', 'portfolio:visitas');
    if (total !== null) resposta.visitas = Number(total);
    const dia = await redis('GET', `portfolio:dia:${hoje}`);
    if (dia !== null) resposta.visitasHoje = Number(dia);
  }

  /* 1b — série dos últimos 14 dias (para o gráfico de tendência) */
  {
    const dias = [];
    const chaves = [];
    for (let i = 13; i >= 0; i--) {
      ...
      dias.push(iso);
      chaves.push(`portfolio:dia:${iso}`);
    }
    const vals = await redis.apply(null, ['MGET'].concat(chaves));
    if (Array.isArray(vals)) {
      resposta.serie = dias.map((d, i) => ({
        dia: d,
        visitas: vals[i] === null ? 0 : Number(vals[i]),
      }));
    }
  }
